...
void counting_sort(vector<Edge> &edges,int num_edges){      //this refer to the textbook 
    int min=-100;
    int max=100;
    int range=203;
...
void make_set(int* &target, int* &rank,int n){  //this refer to the textbook
    for(int i=0;i<n;i++){
        target[i]=i;
        rank[i]=0;
    }
...
int find_set(int x, int* &leader){    //this refer to the textbook
    if(leader[x]!=x){
...
void Union(int x, int y, int* &leader,int* &rank){      //this refer to the textbook
...
bool cycle_detect(vector<Edge> tree,int acyclic_total_num,int end, int start){      //this refer to the textbook and chatgpt for how to use the deque and find
    deque<int> chain;
